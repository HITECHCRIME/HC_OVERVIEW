...
Motifs: dead of blight, oracle, triangles, moth's flight, the life's search, hacking reality. Content Font Regime is `_` as `:` or `-`. Use `///` to designate line in code/text/content/header. Use hard brackets `[]` for additional info `[Rough Prototype] / [Zodiacal Series]`. Brutalist architecture and movement, monumental futura, dirty cyberpunk, self-improvement themes, high technology, minimalism, squad with a goal (hack the world). Use `Hitech Crime presents`? That was one of the initial ideas - to make it feel like a narrative. Moodboard audio series style? Moodboard transitions between songs both in audio and visual. Sequential montages but could be played in a different order or random (hard cut).
